...
# CoreID lineage accept-set (AXIOM_DESIGN_CoreUpgradeMigration.md §11): the blessed
# prior-CoreID set is compiled into the GUEST (CoreID-BOUND), so read it from the
# committed file and export it BEFORE the build — the blessing is baked into the built
# ELF automatically (no manual env needed). The RISC-V build is NOT bit-reproducible,
# so the CoreID's authority is BLAKE3 of the COMMITTED ELF, never a rebuild.
# Absent/empty file => no blessed priors (current-only).
BLESSED_FILE="$SRC_DIR/artifacts/BLESSED_PRIOR_CORE_IDS.txt"
if [ -f "$BLESSED_FILE" ]; then
    AXIOM_BLESSED_PRIOR_CORE_IDS="$(tr -d '[:space:]' < "$BLESSED_FILE")"
    export AXIOM_BLESSED_PRIOR_CORE_IDS
    [ -n "$AXIOM_BLESSED_PRIOR_CORE_IDS" ] && echo "  Blessed priors: $AXIOM_BLESSED_PRIOR_CORE_IDS"
fi

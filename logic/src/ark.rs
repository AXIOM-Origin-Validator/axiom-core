...
//! flood (loss of connectivity → return to dry land) and a backronym —
//! **A**XIOM **R**ecoverable **K**, where K is the `k` (validator-witness
//! threshold) that Ark value is always recoverable back to. An "Ark wallet"
//! is a wallet whose `wallet_id` encodes the Ark security tier
//! (`K_ARK=0, PROOF_TYPE_ARK`; see `wallet_id.rs::WALLET_ID_PARAMS`) — it is
//! a wallet identified by its wallet_id, NOT a separate key pair. Offline
//! value is never un-backed; it goes recoverable, waiting for witnessing
//! (see the load → flood → recede lifecycle below).
//!

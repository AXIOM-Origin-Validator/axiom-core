...
impl FactLink {
    ...
    /// Resolved if: (nabla_confirmation present OR burn_proof present) AND
    /// every inherited scar has a resolution (YPX-001 §1.5.1a — 2026-07-12).
    /// Scars can ONLY be healed by NablaConfirmation or Burn — never by time;
    /// inherited scars only by the ORIGIN txid's Nabla attestation (re-register
    /// or burn). Because every compression path gates on this predicate, a
    /// link carrying unresolved inherited taint can never be checkpointed —
    /// the wash-out defence in depth.
